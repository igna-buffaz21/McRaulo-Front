...
export function PedidosListos() {
  ...
  async function obtenerPedidosPendientes() {
    try {
        const response = await pedidoService.ObtenerPedidosListos();
  ...
  async function cambiarAPreparacion(id_pedido: number) {
    try {
        const response = await pedidoService.cambiarEstadoPedido(id_pedido, ESTADO_PEDIDO.ENTREGADO)
      ...
      <SidebarInset>
        ...
        <div className="p-6">
          <div className="space-y-2 text-center mb-6">
            <h2 className="text-2xl font-bold">Pedidos Listos</h2>
              ...
              <TableBody>
                {pedidos.map((pedido) => (
                  <TableRow key={pedido.id_pedido}>
                    ...
                    <TableCell>{formatearTipo(pedido.tipo)}</TableCell>
